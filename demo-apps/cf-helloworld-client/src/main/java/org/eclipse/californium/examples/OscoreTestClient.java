...
/**
 * OSCORE test client that registers an observe relation on the test server.
 */
public class OscoreTestClient {
	...
	// OSCORE context parameters (must match server configuration)
	...
	public void setClientContext() {
		...
		try {
			...
			System.err.println("Failed to set client OSCORE context: " + e.getMessage());
		}
	}
	...
	/**
	 * Registers an observe relation on /oscore/observe2, waits for 4
	 * notifications, then deregisters and prints the final response.
	 */
	public void observe() throws InterruptedException {
		...
		deregisterRequest.getOptions().setObserve(1);
